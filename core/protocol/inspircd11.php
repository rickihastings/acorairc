...
/*
* Acora IRC Services
* core/protocol/inspircd11.php: Provides support for InspIRCd 1.1
* 
...
class ircd implements protocol
{
	...
	static public $ircd = 'InspIRCd 1.1';
	...
	*/
	...
	{
		self::send( ':'.core::$config->server->name.' VERSION :acora-'.core::$version.' '.self::$ircd.' booted: '.date( 'F j, Y, g:i a', core::$network_time ).'' );
		// ooh, version?
	}
	...
	*/
	static public function introduce_client( $nick, $ident, $hostname, $gecos, $enforcer = false )
	{
		...
		if ( $enforcer )
		{
			self::send( ':'.core::$config->server->name.' NICK '.core::$times[$nick].' '.$nick.' '.$hostname.' '.$hostname.' '.$ident.' '.self::$service_modes['enforcer'].' '.core::$config->conn->vhost.' :'.$gecos );
			// this just connects a psuedoclient.
		}
		else
		{
			self::send( ':'.core::$config->server->name.' NICK '.core::$times[$nick].' '.$nick.' '.$hostname.' '.$hostname.' '.$ident.' '.self::$service_modes['service'].' '.core::$config->conn->vhost.' :'.$gecos );
			// this just connects a psuedoclient.
			self::send( ':'.$nick.' OPERTYPE Service' );
		...
		// add it to the array.
		// i'd prefer not to do this, but because of ircd's that use a UID server
		// protocol craq i'm gonna have to.. total waste of memory..
	...
	*/
	static public function umode( $nick, $user, $mode )
	{
		...
		self::send( ':'.$nick.' SVSMODE '.$user.' '.$mode );
	}
	...
	*/
	...
	{
		self::send( ':'.core::$config->server->name.' METADATA '.$nick.' accountname :'.$nick );
	}
	...
	*/
	...
	{
		self::send( ':'.core::$config->server->name.' METADATA '.$nick.' accountname :' );	
	}
	...
	*/
	static public function on_connect( $ircdata )
	{
		...
		{
			core::alog( 'on_connect(): '.$ircdata[3].' connected to '.substr( $ircdata[0], 1 ), 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_quit( $ircdata )
	{
		...
		{
			core::alog( 'on_quit(): '.substr( $ircdata[0], 1 ).' quit', 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_fhost( $ircdata )
	{
		...
		{
			core::alog( 'on_fhost(): '.substr( $ircdata[0], 1 ).'\'s host changed to '.$ircdata[2], 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_join( $ircdata )
	{
		...
		{
			core::alog( 'on_join(): '.substr( $ircdata[0], 1 ).' joined '.$ircdata[2], 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_part( $ircdata )
	{
		...
		{
			core::alog( 'on_part(): '.substr( $ircdata[0], 1 ).' left '.$ircdata[2], 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_mode( $ircdata )
	{
		...
		{
			core::alog( 'on_mode(): '.substr( $ircdata[0], 1 ).' set '.core::get_data_after( $ircdata, 4 ).' on '.$ircdata[2], 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_kick( $ircdata )
	{
		...
		{
			core::alog( 'on_kick(): '.substr( $ircdata[0], 1 ).' kicked '.$ircdata[3].' from '.$ircdata[2], 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_oper_up( $ircdata )
	{
		...
		{
			core::alog( 'on_oper_up(): '.substr( $ircdata[0], 1 ).' opered up to '.str_replace( '_', ' ', $ircdata[2] ), 'BASIC' );
			// i added this to make debbuing a bit more useful.
	...
	*/
	static public function on_nick_change( $ircdata )
	{
		if ( isset( $ircdata[1] ) && $ircdata[1] == 'NICK' && count( $ircdata ) == 3 )
		{
			core::alog( 'on_nick_change(): '.substr( $ircdata[0], 1 ).' changed nick to '.$ircdata[2], 'BASIC' );
			// debug info
	...
	*/
	static public function on_ident_change( $ircdata )
	{
		...
		{
			core::alog( 'on_ident_change(): '.substr( $ircdata[2], 1 ).' changed ident to '.substr( $ircdata[3], 1 ), 'BASIC' );
			// debug info
		...
		{
			core::alog( 'on_ident_change(): '.substr( $ircdata[0], 1 ).' changed ident to '.substr( $ircdata[2], 1 ), 'BASIC' );
			// debug info
